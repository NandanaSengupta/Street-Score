...
readtable('image_id_by_city.csv', 
...
% sanity check
check = features10(:,1);
check= table2array(check);
length(ismember([1:length(C)], check)) % only these images have feature, image and location data
...
scatter( p_vec(d_vec<10 & p_vec>-1), d_vec(d_vec<10 & p_vec>-1))
xlabel('Pij values')
